...
/**
 * 【デバッグ】ゲーム画面の初期化を行う関数です
 * @param gameManager
 */
export const sceneDebugInitGame = (gameManager: GameManager) => {
  ...
  document.removeEventListener("keydown", (ev) => {
    if (ev.code === "KeyK") gameManager.debugMethod().killEnemy();
  });
  document.addEventListener("keydown", (ev) => {
    if (ev.code === "KeyK") {
      console.log("K");
      console.log(gameManager.gameState.enemies);
      gameManager.debugMethod().killAllEnemy();
    }
  });
  gameManager.debugMethod().god();
